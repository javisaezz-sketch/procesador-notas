...
const STORAGE_KEY_PREFIX = 'categoria-default-';

export function getCategoriaDefaultMedio(medio) {
  const categorias = getCategoriasMedio(medio);
  const fromJson = categorias.find((cat) => cat.default)?.slug;
  if (fromJson) return fromJson;

  if (typeof window !== 'undefined' && medio?.slug) {
    const saved = localStorage.getItem(`${STORAGE_KEY_PREFIX}${medio.slug}`);
    if (saved && categorias.some((cat) => cat.slug === saved)) {
      return saved;
    }
  }

  return categorias[0]?.slug ?? null;
}

export function guardarCategoriaDefaultMedio(medio, slug) {
  if (typeof window !== 'undefined' && medio?.slug && slug) {
    localStorage.setItem(`${STORAGE_KEY_PREFIX}${medio.slug}`, slug);
  }
}
...
/** Medios WordPress conectados al procesador (misma lógica de publicación e imagen destacada). */
export const MEDIOS_WORDPRESS = MEDIO_ORDEN_SLUGS;

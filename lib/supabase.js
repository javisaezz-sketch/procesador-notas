...
import { firmarUrlImagenStorage } from './urlsImagenStorage.js';
...
export function createSupabaseClient() {
  ...
  if (!url || !key) {
    throw new Error(
      'Faltan SUPABASE_URL y SUPABASE_SERVICE_ROLE_KEY (o SUPABASE_ANON_KEY). Comprueba tu .env.',
    );
  }
...
async function firmarImagenesArticulos(articulos) {
  ...
  return Promise.all(
    articulos.map(async (articulo) => {
      ...
      return {
        ...articulo,
        imagen_destacada_url: await firmarUrlImagenStorage(
          supabaseAdmin,
...
async function getArticulosPorEstado(estado) {
  ...
  const enriched = await enrichArticulos(articulos ?? []);
  return firmarImagenesArticulos(enriched);
}

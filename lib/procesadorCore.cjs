...
`.trim();
  ...
  const urlsImagenesNota = imagenesNota.map((imagen) => imagen.url).filter(Boolean);
  const imagenDestacadaUrl = urlsImagenesNota[0] ?? null;
  ...
  return {
    ...
    imagenesAdicionales: Math.max(0, urlsImagenesNota.length - 1),
  };
}

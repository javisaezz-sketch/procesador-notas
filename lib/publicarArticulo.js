...
export async function getArticuloParaPublicar(articuloId) {
  ...
  const { data: articulo, error } = await supabase
    .from('articulos')
    .select('id, titulo_generado, contenido_generado, estado, medio_id, imagen_destacada_url, nota_prensa_id')
    .eq('id', articuloId)
    .single();
  ...
  return {
    articulo: {
      ...articuloSync,
      email_notificacion: resolverEmailNotificacionDesdeNota(nota),
...
export async function marcarArticuloComoPublicado(articuloId) {
  const supabase = createSupabaseClient();
...
export async function publicarArticulo(articuloId, categoriaSlug) {
  ...
  const articuloActualizado = await marcarArticuloComoPublicado(articuloId);

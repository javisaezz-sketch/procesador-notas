...
// FUNCIÓN CORREGIDA: Elimina galeria-nota al final, inyecta fotos y LIMPIA marcadores siempre
function inyectarImagenesContextuales(html, imagenesUrls, titulo) {
  ...
  // 2. Insertar de forma contextual ÚNICAMENTE si hay imágenes disponibles
  ...
  // 3. LIMPIEZA TOTAL: Borra cualquier marcador [IMAGEN_X] sobrante que haya puesto Gemini
...
`.trim();
  ...
  // CAMBIO CLAVE: Se ejecuta SIEMPRE para inyectar fotos O limpiar marcadores [IMAGEN_X]
  ...
  return {
    ...
    imagenesAdicionales: imagenes.adicionalesUrls ? imagenes.adicionalesUrls.length : 0,
  };
}

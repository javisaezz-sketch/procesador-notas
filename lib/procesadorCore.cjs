...
async function procesarUnaNota(supabase, genAI, nota) {
  ...
  const { buscarNotaDuplicada, buscarArticuloDeNota } = await import(
    './detectarDuplicadoNota.js'
  );

  const articuloExistente = await buscarArticuloDeNota(supabase, nota.id, medio.id);

  if (articuloExistente) {
    await supabase
      .from('notas_prensa')
      .update({ estado: 'procesada', contenido_html: null })
      .eq('id', nota.id);

    return {
      duplicado: true,
      motivoDuplicado: 'articulo_existente',
      articuloId: articuloExistente.id,
      notaId: nota.id,
      medioId: medio.id,
      medioNombre: medio.nombre,
      titulo: articuloExistente.titulo_generado || getNotaTitulo(nota),
      imagen: null,
      imagenesAdicionales: 0,
    };
  }

  const duplicado = await buscarNotaDuplicada(supabase, {
    medioId: medio.id,
    asunto: nota.asunto,
    contenidoOriginal: nota.contenido_original,
    excluirNotaId: nota.id,
  });

  if (duplicado) {
    const articuloPrevio = await buscarArticuloDeNota(
      supabase,
      duplicado.nota.id,
      medio.id,
    );
    ...
    if (articuloPrevio) {
      await supabase.from('notas_prensa').update({ estado: 'descartada' }).eq('id', nota.id);

      return {
        duplicado: true,
        motivoDuplicado: duplicado.motivo,
        articuloId: articuloPrevio.id,
        notaId: nota.id,
        notaDuplicadaDe: duplicado.nota.id,
        medioId: medio.id,
        medioNombre: medio.nombre,
        titulo: articuloPrevio.titulo_generado || getNotaTitulo(nota),
        imagen: null,
        imagenesAdicionales: 0,
      };
    }
...
`.trim();
  ...
  const { listarImagenesDeNota } = await import('./imagenes.js');
  const {
    quitarImagenIncrustada,
    quitarImagenesGaleria,
    inyectarImagenesContextuales,
  } = await import('./contenidoHtml.js');

  const imagenesNota = await listarImagenesDeNota(supabase, nota.id, {
  ...
  // Solo limpiar marcadores; las imágenes se inyectan al publicar según selección del dashboard
  contenidoFinal = inyectarImagenesContextuales(contenidoFinal, [], titulo_generado);

  const { elegirImagenDestacadaDeNota } = await import('./imagenes.js');
  const imagenDestacadaUrl = elegirImagenDestacadaDeNota(imagenesNota);
  const totalImagenesNota = imagenesNota.filter((imagen) => imagen.url).length;
  ...
  const insertPayload = {
    ...
    imagen_destacada_url: imagenDestacadaUrl,
    imagenes_publicar_urls: imagenDestacadaUrl ? [imagenDestacadaUrl] : [],
  ...
  return {
    ...
    imagen: imagenDestacadaUrl,
    imagenesAdicionales: Math.max(0, totalImagenesNota - 1),
  };
}
  ...
  while (procesadasEnRun < MAX_NOTAS_POR_RUN) {
    ...
    try {
      ...
      if (resultado.duplicado) {
        ...
        console.log(
          `   ⏭️  Nota #${nota.id}: duplicado (${describirMotivoDuplicado(resultado.motivoDuplicado)}) → artículo #${resultado.articuloId}`,
        );
        continue;
      }

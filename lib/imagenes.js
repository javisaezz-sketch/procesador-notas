function puntuarImagenDestacada(imagen) {
  const nombre = String(imagen.nombre_archivo || imagen.url || '').toLowerCase();
  const url = String(imagen.url || '').toLowerCase();
  let score = 0;

  if (/\.jpe?g(\?|$)/.test(nombre) || /\.jpe?g(\?|$)/.test(url)) score += 40;
  else if (/\.png(\?|$)/.test(nombre) || /\.png(\?|$)/.test(url)) score += 30;
  else if (/\.webp(\?|$)/.test(nombre) || /\.webp(\?|$)/.test(url)) score += 10;

  const match = nombre.match(/-(\d+)\.(jpe?g|png|webp|gif)$/i);
  if (match) {
    score += Math.max(0, 12 - Number(match[1]));
  }

  if (/og|featured|destacad|hero|principal|cover|cabecera|thumbnail/i.test(nombre)) {
    score += 25;
  }

  if (/thumb|mini|icon|logo|emoji|sprite|badge/i.test(nombre)) {
    score -= 50;
  }
...
export function elegirImagenDestacadaDeNota(imagenes = []) {
  if (!imagenes.length) return null;

  const ordenadas = [...imagenes].sort((a, b) => {
    const diff = puntuarImagenDestacada(b) - puntuarImagenDestacada(a);
...
export async function resolverImagenesParaArticulo(
  ...
  } = await import('./limpiarAlmacenNota.js');
  ...
  if (Array.isArray(imagenesPublicarUrls)) {
    ...
    const adicionalesUrls = seleccionadas.filter(
  ...
  if (imagenDestacadaUrl) {
    const destacadaUrl =
      canonicalizarUrlImagen(imagenDestacadaUrl, urlsDisponibles) ??
      imagenDestacadaUrl;
...
export async function aplicarImagenesAlArticulo(supabase, articulo, nota) {
  const { urlsImagenesCuerpo, mismaImagenStorage } = await import(
    './limpiarAlmacenNota.js'
  );
  ...
  const soloDestacadaLegacy =
    !seleccionManual && Boolean(imagenes.destacadaUrl);
  ...
  const urlsCuerpo =
    seleccionManual || soloDestacadaLegacy ? imagenes.adicionalesUrls : [];

  let contenido = prepararContenidoConImagenesSeleccionadas(
  ...
  if ((seleccionManual || soloDestacadaLegacy) && imagenes.destacadaUrl) {
    const faltaDestacada = !articulo.imagen_destacada_url;
    const soloCanonizar =
      articulo.imagen_destacada_url &&
      mismaImagenStorage(articulo.imagen_destacada_url, imagenes.destacadaUrl) &&
      articulo.imagen_destacada_url !== imagenes.destacadaUrl;

    if (faltaDestacada || soloCanonizar) {

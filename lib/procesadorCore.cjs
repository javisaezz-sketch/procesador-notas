...
// FUNCIÓN CORREGIDA: Inserta las imágenes contextualmente Y elimina duplicados al final
function inyectarImagenesContextuales(html, imagenesUrls, titulo) {
  let contenido = html;

  // 1. Limpiar cualquier bloque previo de galería o imágenes inyectadas al final
  ...
  // 2. Insertar cada imagen contextualmente
  imagenesUrls.forEach((url, index) => {
    ...
    if (contenido.includes(marcador)) {
      ...
      // Si no encuentra el marcador explícito, la inserta tras un párrafo </p>
      const partes = contenido.split('</p>');
      const numPartes = partes.length;
      if (numPartes > 2) {
        // Calcular posición estratégica entre párrafos
  ...
  // 3. Limpiar cualquier marcador sobrante que la IA haya puesto de más
...
`.trim();
  ...
  // Inyección contextual limpia sin duplicar fotos al final
